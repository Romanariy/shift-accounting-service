...
def ingest_telegram_message(
    ...
    parsed = parse_shift_message(text, aliases=get_all_aliases(), default_year=default_year or date.today().year)
    ...
    shift = ShiftEntry.objects.create(
        ...
        calculated_amount=calculate_shift_amount(work_type, parsed.hours, parsed.date),
    ...
    return IngestResult(
        ...
        companion=companion,
        message=(
            f"Записал: {shift.date:%d.%m.%Y}, {shift.employee_name_snapshot or 'без сотрудника'}, "
            f"{shift.get_work_type_display()}, сумма {shift.calculated_amount} руб.{companion_note}.{review_note}"
        ),
