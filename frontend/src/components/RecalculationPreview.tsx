...
export default function RecalculationPreview({preview,busy,onClose,onApply}: {preview:any;busy:boolean;onClose:()=>void;onApply:()=>void}) {
  return <section className="panel"><div className="panel-heading"><h2>Предпросмотр пересчёта · {preview.changes.length} изменений</h2><button className="icon-button" onClick={onClose} title="Закрыть пересчёт" aria-label="Закрыть пересчёт">×</button></div>
    <div className="padded"><p>Проверьте суммы участников. Применение обновит тарифы выбранного периода и переведёт исторические почасовые смены на общий расчёт.</p>
