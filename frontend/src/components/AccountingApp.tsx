...
import Payments, { PaymentStatus, PaymentsData } from "./Payments";
...
const dateLabel = (v: string) => v ? v.slice(0,10).split("-").reverse().join(".") : "—";
...
export default function AccountingApp() {
  ...
  const [batchOrdering,setBatchOrdering] = useState("-id");
  ...
  const refreshPayments = useCallback(async(signal?:AbortSignal,background=false)=>{
    ...
    try {
      const query=new URLSearchParams({state:page==="payments"?paymentState:"open",offset:String(page==="payments"?paymentOffset:0),limit:String(page==="payments"?pageSize:1)});
      const response=await fetch(base+"payments/?"+query,{signal,cache:"no-store"});
      ...
      if(page==="payments"&&paymentOffset>0&&paymentOffset>=result.count){setPaymentOffset(Math.max(0,Math.floor((result.count-1)/pageSize)*pageSize));return;}
  ...
  },[page,paymentState,paymentOffset]);
  useEffect(()=>{
  ...
  const load = useCallback(async()=>{
    ...
    if(page==="payments") {const bootstrap=await api("bootstrap/");if(sequence===requestSequence.current)setData(bootstrap);return;}
    if(page==="earnings") {
    ...
    if(page==="invoices") {batchQuery.set("month",month);batchQuery.set("ordering",batchOrdering);if(org)batchQuery.set("organization",org);if(batchState)batchQuery.set("state",batchState);}
  ...
  async function prepareBatch(replaces?:Item) {const ids=replaces?.organization_ids || (org?[Number(org)]:orgOptions.map(o=>o.value));if(!ids.length){setError("Сначала добавьте организацию.");return;}const result=await run(()=>api("batches/","POST",{start:replaces?.start||start,end:replaces?.end||end,organizations:ids,replaces:replaces?.id}),"Отчёты подготовлены. Проверьте их перед отправкой.");if(result){setSelectedBatch(result.id);setMonth(result.start.slice(0,7));setBatchState("");setPage("invoices");}}
  function openBatchEditor() {setEditor({title:"Подготовить отчёты",resource:"batch-draft",values:{start,end,organizations:org?[Number(org)]:orgOptions.map(o=>o.value)},fields:[{key:"start",label:"Начало периода",type:"date",required:true},{key:"end",label:"Конец периода",type:"date",required:true},{key:"organizations",label:"Организации",type:"multi",options:orgOptions,hint:"Выберите одну или несколько организаций. Для нескольких используйте Ctrl / Cmd."}]});}
  ...
  async function openPaymentInvoice(invoice:Item) {
    ...
    try {const batch=await api(`batches/${invoice.batch}/`);setLinkedBatch(batch);setSelectedBatch(batch.id);setMonth(invoice.start.slice(0,7));setOrg("");setBatchState("");setOffset(0);setPage("invoices");}
    catch(error) {setError(error instanceof Error?error.message:"Не удалось открыть счёт");}
  ...
  const linkedVisible=linkedBatch&&linkedBatch.id===selectedBatch&&linkedBatch.start.slice(0,7)<=month&&linkedBatch.end.slice(0,7)>=month&&(!org||linkedBatch.invoices.some((invoice:Item)=>String(invoice.organization)===org))&&(!batchState||linkedBatch.state===batchState);
  const selected=batches.find(b=>b.id===selectedBatch) || (linkedVisible?linkedBatch:null) || batches[0];
  const titles:Record<string,[string,string]>={earnings:["Главный отчёт","Заработок каждого сотрудника по всем организациям."],overview:["Всё сходится.","Работы, расходы и счета — в одном месте."],records:["Журнал работ","Типовые услуги и разовые задачи вашей команды."],expenses:["Расходы","Каждая покупка привязана к своей организации."],payments:["Оплаты","Счета организаций, ожидающие подтверждения получателей в Telegram."],invoices:["Счета и отчёты","Проверяйте детали. Отправляйте только после подтверждения."],services:["Услуги и тарифы","Ваши правила расчёта — для каждой организации."],settings:["Настройки","Организации, команда и подключение Telegram."]};
  ...
  return <div className="accounting"><aside className="sidebar"><Link className="brand" href="/">свод<span>учёт студий</span></Link><nav>{[["overview","Обзор"],["records","Журнал работ"],["expenses","Расходы"],["invoices","Счета"],["payments","Оплаты"],["earnings","Главный отчёт"],["services","Услуги и тарифы"],["settings","Настройки"]].map(([id,title])=><button key={id} className={page===id?"active":""} onClick={()=>{setPage(id);setOffset(0);setSearch("");}}><Icon name={id}/>{title}{id==="payments"&&<span className="payment-count" aria-label={`Незакрытых счетов: ${payments.open_count}`}>{payments.open_count}</span>}</button>)}</nav><div className="sidebar-foot"><span className="status-dot"/>Екатеринбург · UTC+5<br/><span>Работа учтена.<br/>Всё под контролем.</span></div></aside>
    ...
    {page==="payments"&&<Payments data={payments} state={paymentState} offset={paymentOffset} pageSize={pageSize} loading={paymentLoading} error={paymentError} onState={value=>{setPaymentState(value);setPaymentOffset(0);}} onOffset={setPaymentOffset} onOpen={openPaymentInvoice} contacts={data?.contacts||[]}/>}
    ...
    {editor?.resource==="records"&&editor.values.kind==="service"?<ServiceRecordEditor key={editor.resource+String(editor.values.id)} editor={editor} data={data} busy={busy} error={error} onClose={()=>setEditor(null)} onSave={saveEditor}/>:editor&&<Modal key={editor.resource+String(editor.values.id)} editor={editor} busy={busy} error={error} onClose={()=>setEditor(null)} onSave={async values=>{if(editor.resource==="batch-draft"){const result=await run(()=>api("batches/","POST",values));if(result){setSelectedBatch(result.id);setMonth(result.start.slice(0,7));setBatchState("");setEditor(null);}}else if(editor.resource.startsWith("adjustment:")){const id=Number(editor.resource.split(":")[1]);const inv=batches.flatMap(b=>b.invoices).find(i=>i.id===id);const result=await run(()=>api(`invoices/${id}/`,"PUT",{adjustments:[...(inv?.adjustments||[]),values]}));if(result)setEditor(null);}else await saveEditor(values);}}/>}
    </main></div>;
}
