...
export default function AccountingApp() {
  ...
  async function saveEditor(values:Record<string,any>) {if(!editor)return;if(editor.resource==="settings"){const {offer_enabled,offer_chat_id,offer_thread_id,...rest}=values;values={...rest,offer_config:{enabled:!!offer_enabled,chat_id:offer_chat_id,thread_id:offer_thread_id}};}const path=editor.resource+"/"+(values.id && editor.resource!=="settings"?values.id+"/":"");const result=await run(()=>api(path,values.id || editor.resource==="settings"?"PUT":"POST",values,editor.legacy));if(result)setEditor(null);}
  ...
  titles.offers = ["Предложения смен", "Планирование по скриншотам YCLIENTS и распределение сотрудников."];
